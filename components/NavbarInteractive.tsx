...
import { NavbarWishlistLink } from "@/components/NavbarWishlistLink";
...
const CommandSearch = dynamic(() => import("@/components/CommandSearch").then((mod) => mod.CommandSearch));
...
export function NavbarInteractive({
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      toggleCmsEditMode: state.toggleCmsEditMode,
      openCmsSection: state.openCmsSection,
      solutionsMenuDraft: state.cmsDrafts.solutionsMenu,
    })),
  );
  ...
  useEffect(() => {
    ...
    header.classList.toggle("is-mobile-nav-open", mobileNavOpen);
  ...
  useEffect(() => {
    ...
    menuDefinitions.forEach((menuItem) => {
      hrefs.add(menuItem.href);
    });
  ...
  useEffect(() => {
    ...
    if (activeMenu === "products") {
      productMenuColumns.forEach((column) => {
        column.links.slice(0, 4).forEach((linkItem) => {
          hrefs.add(linkItem.href);
        });
        hrefs.add(column.preview.href);
      });
      hrefs.add(productsMenuFeaturedPanel.footerCtaHref);
    }

    if (activeMenu === "solutions") {
      solutionsMenuDraft.columns.forEach((column) => {
        column.links.slice(0, 4).forEach((linkItem) => {
          hrefs.add(linkItem.href);
        });
        hrefs.add(column.preview.href);
      });
      hrefs.add(solutionsMenuDraft.featuredPanel.footerCtaHref);
    }

    if (activeMenu === "industries") {
      industriesMenuLinks.forEach((linkItem) => {
        hrefs.add(linkItem.href);
      });
    }

    if (activeMenu === "company") {
      companyMenuLinks.forEach((linkItem) => {
        hrefs.add(linkItem.href);
      });
    }
  ...
  }, [activeMenu, router, solutionsMenuDraft.columns, solutionsMenuDraft.featuredPanel.footerCtaHref]);
    ...
    <header className="site-header" id="site-header">
      ...
      <div className="container">
        <nav className="nav" aria-label="Primary navigation">
          ...
          <div className="nav-zone nav-zone-center" id="nav-mobile-panel" ref={navRef}>
