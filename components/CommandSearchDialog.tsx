...
export function CommandSearchDialog({
  ...
  const resultGroups = useMemo<CommandResultGroup[]>(() => {
    if (normalizedQuery.length < MIN_SEARCH_LENGTH) {
      return [{ label: "Quick Links", items: quickLinks }];
    }

    const productItems: CommandResult[] = productsList
      .map((product) => {
    ...
    const solutionItems: CommandResult[] = solutionsList
      .map((solution) => {
    ...
    const industryItems: CommandResult[] = industriesList
      .map((industry) => {
  ...
  useEffect(() => {
    ...
    const hrefs = new Set<string>(quickLinks.map((item) => item.href));
    flattenedResults.slice(0, 8).forEach((result) => {
  ...
  }, [flattenedResults, isOpen, router]);
  ...
  return createPortal(
    <div className="command-search-backdrop" onClick={onClose}>
      <div
